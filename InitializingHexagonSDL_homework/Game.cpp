...
void Game::render() {
	...
	SDL_SetRenderDrawColor(renderer, 0x00, 0x00, 0xFF, 0xFF); // Centered blue line
	//SDL_RenderDrawLine(renderer, 0, wh / 2, ww, wh / 2);

	//Shapes::drawPentagon(renderer, 300, 200, 200);
	Shapes::drawHexagon(renderer, 300, 200, 200);

	/*SDL_Rect fillRect = {ww / 4, wh / 4, ww / 2, wh / 2};     //fill Rectangle
	SDL_SetRenderDrawColor(renderer, 0xFF, 0x00, 0x00, 0xFF); // red color
	SDL_RenderFillRect(renderer, &fillRect);

	SDL_Rect outlineRect = {ww / 6, wh / 6, ww * 2 / 3, wh * 2 / 3}; // Rectangle Outline
	...
	SDL_SetRenderDrawColor(renderer, 0xFF, 0xFF, 0x00, 0x00); // yellow color line
